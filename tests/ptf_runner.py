...
def ptf_runner(host, testdir, testname, platform_dir=None, params={},
    ...
    try:
        if pdb:
            ...
            import pdb
            pdb.set_trace()
        # Always drop a runnable script onto the PTF host so the test can be
        # replayed by hand (useful for setting pdb breakpoints in ptftests/).
        try:
            short_name = testname.split(".")[-1]
            replay_script_local = "/tmp/run_{}.sh".format(short_name)
            with open(replay_script_local, "w") as f:
                f.write("#!/bin/bash\n")
                f.write("# Auto-generated by tests/ptf_runner.py - replay the last\n")
                f.write("# {}.{} invocation directly on the PTF host.\n".format(testdir, testname))
                f.write("set -e\n")
                f.write("cd /root\n")
                f.write(cmd + "\n")
            host.copy(src=replay_script_local, dest="/root/", mode="0755")
            logger.info("Wrote replay script to ptf host: /root/run_{}.sh".format(short_name))
        except Exception as e:
            logger.warning("Failed to write replay script for {}: {}".format(testname, e))
