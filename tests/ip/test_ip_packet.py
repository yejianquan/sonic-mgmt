...
class TestIPPacket(object):
    ...
    def sum_portstat_ifaces_counts(portstat_out, ifaces, column):
        logging.info("ifaces_is: %s" % ifaces)
        if len(ifaces) == 0:
            return 0
        if len(ifaces) == 1:
    ...
    def parse_interfaces(output_lines, pc_ports_map):
        ...
        """
        logging.info("output_lines_is: %s" % output_lines)
        route_targets = []
        ifaces = []
        ...
        logging.info("parse_interfaces_is: %s" % ifaces)
        return ifaces

    @pytest.fixture(scope="class")
    def test_param(self, duthost, tbinfo):
        mg_facts = duthost.get_extended_minigraph_facts(tbinfo)
        self.skip(mg_facts)
        logging.info("mg_facts_is: %s" % mg_facts)
        pc_ports_map = {}
        ...
        logging.info(
            "selected_peer_ip_ifaces_pairs is: %s, ptf_port_idx is %s" % (selected_peer_ip_ifaces_pairs, ptf_port_idx))

        yield selected_peer_ip_ifaces_pairs, ptf_port_idx, pc_ports_map, mg_facts["minigraph_ptf_indices"]

    def test_relay_ip_packet_with_0x0000_chksum(self, duthost, ptfadapter, test_param):
        # GIVEN a ip packet with checksum 0x0000(compute from scratch)
        # WHEN send the packet to DUT
        # THEN DUT should relay it as normal ip packet

        (peer_ip_ifaces_pair, ptf_port_idx, pc_ports_map, ptf_indices) = test_param
        ptf_port_idx = 29
        ...
        logging.info("portstat_out: %s" % (portstat_out))
        ...
        logging.info("intf_status: %s, %s, %s" % (rx_ok, rx_drp, tx_ok))

        assert (self.PKT_NUM_MIN <= match_cnt <= self.PKT_NUM_MAX)
        assert (self.PKT_NUM_MIN <= rx_ok <= self.PKT_NUM_MAX)
        assert (self.PKT_NUM_MIN <= tx_ok <= self.PKT_NUM_MAX)

    def test_relay_ip_packet_with_0xffff_chksum(self, duthost, ptfadapter, test_param):
        # GIVEN a ip packet with checksum 0x0000(compute from scratch)
        # WHEN manually set checksum as 0xffff and send the packet to DUT
        # THEN DUT should (both of them are fine):
        #   a. Tolerant packet with 0xffff, relay it as normal packet
        #   b. Drop packet with 0xffff and add drop count
        ...
        tolerated = (self.PKT_NUM_MIN <= match_cnt <= self.PKT_NUM_MAX) \
                    and (self.PKT_NUM_MIN <= rx_ok <= self.PKT_NUM_MAX) \
                    and (self.PKT_NUM_MIN <= tx_ok <= self.PKT_NUM_MAX)

        dropped = (self.PKT_NUM_MIN <= rx_ok <= self.PKT_NUM_MAX) \
                  and (self.PKT_NUM_MIN <= rx_drp <= self.PKT_NUM_MAX)
        assert (tolerated or dropped)

    def test_relay_ip_packet_recomputed_0xffff_chksum(self, duthost, ptfadapter, test_param):
        # GIVEN a ip packet, after relay(ttl-1) by DUT,
        ...
        # WHEN send the packet to DUT
        # THEN DUT recompute new checksum correctly and relay packet as expected.
    ...
    def test_relay_ip_packet_recomputed_0x0000_chksum(self, duthost, ptfadapter, test_param):
        # GIVEN a ip packet, after relay(ttl-1) by DUT, it's checksum will be 0x0000 after recompute from scratch
        # WHEN send the packet to DUT
        # THEN DUT recompute new checksum as 0x0000 and relay packet as expected.
    ...
    def test_relay_normal_ip_packet(self, duthost, ptfadapter, test_param):
        # GIVEN a random normal ip packet
        # WHEN send the packet to DUT
        # THEN DUT should relay it as normal ip packet, nothing change but ttl-1
        (peer_ip_ifaces_pair, ptf_port_idx, pc_ports_map, ptf_indices) = test_param
        peer_ip_ifaces_pair = [(u'10.0.0.59', [u'Ethernet116']), (u'10.0.0.61', [u'Ethernet120'])]
        ptf_port_idx = 29
        pkt = testutils.simple_ip_packet(
            eth_dst=duthost.facts["router_mac"],
            eth_src=ptfadapter.dataplane.get_mac(0, ptf_port_idx),
        ...
        exp_pkt = mask.Mask(exp_pkt)
        # ether must mask
        # ip no need mask, chksum changed with chksum
    ...
    def test_drop_ip_packet_with_wrong_0xffff_chksum(self, duthost, ptfadapter, test_param):
        ...
        tx_ok = TestIPPacket.sum_portstat_ifaces_counts(portstat_out, peer_ip_ifaces_pair[1][1], "tx_ok")
        logging.info("intf_status: %s, %s, %s" % (rx_ok, rx_drp, tx_ok))

        assert (self.PKT_NUM_MIN <= rx_ok <= self.PKT_NUM_MAX)
        assert (self.PKT_NUM_MIN <= rx_drp <= self.PKT_NUM_MAX)
